 ...
 * (Adaptive Large Neighborhood Search) específicamente diseñados para el
 * problema
 * de logística MoraPack.
 *
 * Los operadores están diseñados para preservar la prioridad de entregas a
 * tiempo.
 */
public class ALNSDestruction {
    ...
    private static double horasRuta(ArrayList<Vuelo> ruta) {
        ...
        double h = 0;
        for (Vuelo f : ruta)
            h += f.getTiempoTransporte();
        if (ruta.size() > 1)
            h += (ruta.size() - 1) * 2.0; // conexiones de 2h
        return h;
    }
    ...
    /**
     * CORRECCIÓN: Slack real - horas disponibles desde fechaPedido menos horas de
     * la ruta actual
     * REFINAMIENTO: Clampar slack negativo por deadlines raros (deadline <
     * fechaPedido)
     */
    ...
    /**
     * CORRECCIÓN: Destrucción aleatoria mejorada - sesgo por mayor slack y más
     * productos
     */
    public ResultadoDestruccion destruccionAleatoria(
        ...
        for (Map.Entry<Pedido, ArrayList<Vuelo>> e : solucionActual.entrySet()) {
            ...
            // Holgura de tiempo: cuantas Cuántas horas de sobra tiene el pedido antes de su
            // deadline.
            // Cuanto más slack, menos urgente es.
            double slack = slackHoras(p, r);
            int productos = productosDe(p);

            // REFINAMIENTO: Penalizar fuertemente paquetes ya en destino (no liberan
            // capacidad de vuelo)
            if (yaEstaEnDestino(r)) {
                slack = slack * 0.1; // Reducir significativamente su prioridad
            }
            // Los pedidos con mas slack obtienen mayor puntuacion, por lo tanto es mas
            // probable que sean destruidos
        ...
        // Los primeros del listado son los más “prescindibles” (porque tienen mucho
        // margen o son grandes).
        ...
        while (tomados < numDestruir && i < candidatos.size()) {
            ...
            if (ruta == null)
                ruta = new ArrayList<>(); // Protección contra nulos
    ...
    /**
     * Destrucción por zona geográfica: elimina paquetes de un continente
     * específico.
    ...
    public ResultadoDestruccion destruccionGeografica(
        ...
        for (Pedido pedido : solucionActual.keySet()) {
            Continente continenteOrigen = obtenerAeropuerto(pedido.getAeropuertoOrigenCodigo()).getCiudad()
                    .getContinente();
            Continente continenteDestino = obtenerAeropuerto(pedido.getAeropuertoDestinoCodigo()).getCiudad()
                    .getContinente();
        ...
        for (Continente continente : Continente.values()) {
            ...
            for (Pedido pedido : paquetesOrigen) {
                if (obtenerAeropuerto(pedido.getAeropuertoOrigenCodigo()).getCiudad()
                        .getContinente() != obtenerAeropuerto(pedido.getAeropuertoDestinoCodigo()).getCiudad()
                                .getContinente()) {
        ...
        // Encontrar paquetes del continente seleccionado y se agregar a la lista de
        // zona afectada
        ArrayList<Pedido> paquetesCandidatoes = new ArrayList<>();
        for (Pedido pedido : solucionActual.keySet()) {
            if (obtenerAeropuerto(pedido.getAeropuertoOrigenCodigo()).getCiudad()
                    .getContinente() == continenteSeleccionado ||
                    obtenerAeropuerto(pedido.getAeropuertoDestinoCodigo()).getCiudad()
                            .getContinente() == continenteSeleccionado) {
                paquetesCandidatoes.add(pedido);
            }
        }
        // si la cantidad de paquetes es menor entonces no es suficiente este criterio y
        // mejor se usa el aleatorio
        // OJO CON ESTO SI ENTRA ACA COMO EL ALGORITMO SABE QUE SE USO PARA AUMENTAR ESA
        // COMBINACION
        ...
        // REFINAMIENTO: Precomputar slack y productos para evitar recalcular en
        // comparator
        ...
        for (Pedido pedido : paquetesCandidatoes) {
            ...
            info.paquete = pedido;
            info.intercontinental = obtenerAeropuerto(pedido.getAeropuertoOrigenCodigo()).getCiudad()
                    .getContinente() != obtenerAeropuerto(pedido.getAeropuertoDestinoCodigo()).getCiudad()
                            .getContinente();
        ...
        // Se eliminan primero los pedidos grandes, no urgentes y que cruzan
        // continentes.

        // Priorizar: 1) intercontinental, 2) NO en destino, 3) mayor slack, 4) más
        // productos
    ...
    /**
     * Destrucción por tiempo: elimina paquetes con deadlines en un rango
     * específico.
    ...
    public ResultadoDestruccion destruccionBasadaEnTiempo(
        ...
        // REFINAMIENTO: Elige grupo prioritariamente, usando enDestino como último
        // recurso
        ...
        // REFINAMIENTO: Ordenar por productos desc para tie-break (más productos = más
        // capacidad liberada)
        ...
        // Barajar parcialmente para diversidad (mantener bias hacia más productos en el
        // top)
        if (grupoSeleccionado.size() > 10) {
            // Solo barajar los últimos elementos, mantener los primeros (más productos)
            // intactos
    ...
    public ResultadoDestruccion destruccionRutaCongestionada(
        ...
        // CORRECCIÓN: Score por paquete basado en congestión crítica + productos -
        // urgencia
        class Candidato {
            Pedido paquete;
            ArrayList<Vuelo> ruta;
        ...
        for (Map.Entry<Pedido, ArrayList<Vuelo>> e : solucionActual.entrySet()) {
            ...
            for (Vuelo f : r) {
                double utilizacion = (f.getCapacidadMaxima() > 0)
                        ? ((double) f.getCapacidadUsada() / f.getCapacidadMaxima())
                        : 0.0;
            ...
            double puntuacion = PESO_UTILIZACION * congestion + PESO_PRODUCTOS * productos
                    - PENALIZACION_SLACK * penalizacionSlack;
            if (puntuacion > 0) {
                Candidato c = new Candidato();
                c.paquete = p;
                c.ruta = r;
